...
public class StaminaFoodItemHandler : IItemGiftHandler
{
    ...
    public bool CanGive(ExpandedItemDefinition item, 
        ...
        bool altGirl)
    {
        if (item.Core.foodType == girl.Core.badFoodTypes[0]) return false;

        if (Game.Session.Puzzle.puzzleStatus.foodGiftCount >= 2)
        {
            return false;
        }
