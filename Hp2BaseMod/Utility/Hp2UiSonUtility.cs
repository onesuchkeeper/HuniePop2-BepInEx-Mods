    ...
    internal static class Hp2UiSonUtility
    {
        ...
        internal static void MakeDefaultDataDotCs(GameData gameData)
        {
            try
            {
                ...
                {
                    file.WriteLine($"       public static readonly IEnumerable<string> {entry.Key} = new string[] {{{ToCsv(entry.Value.Skip(1), _addQuotesFormat)}}};");
                }
                ...
                {
                    file.WriteLine($"       public static readonly IEnumerable<string> {entry.Key} = new string[] {{{ToCsv(entry.Value.Invoke(gameData).Skip(1), _addQuotesFormat)}}};");
                }
