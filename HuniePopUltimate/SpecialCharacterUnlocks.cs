...
internal static class SpecialCharacterUnlocks
{
    ...
    {
        if (!Plugin.HasSingleDate) return;

        // Ignore hub-to-hub transition
        if (args.from?.locationType == LocationType.HUB
            && args.to?.locationType == LocationType.HUB)
        {
            return;
        }

        // Ignore dates
        if (args.to?.locationType == LocationType.DATE)
        {
            return;
        }
        ...
        if (VenusOverride(nobodyDef, ref args.girlPairDef))
        {
        ...
        bool overrideTriggered = KyuOverride(nobodyDef, args.to, args.sidesFlipped)
            || CelesteOverride(previousLocId, args.to, args.sidesFlipped)
            || MomoOverride(previousLocId, args.to, args.sidesFlipped);
    ...
    private static bool KyuOverride(GirlDefinition nobodyDef, LocationDefinition locationDef, bool sidesFlipped)
    {
        if (Game.Session.Puzzle.puzzleStatus.bonusRound)
        {
            var kyuPair = Game.Persistence.playerFile.GetPlayerFileGirlPair(ModInterface.GameData.GetGirlPair(Pairs.KyuSingleDate));
    ...
    private static bool VenusOverride(GirlDefinition nobodyDef, ref GirlPairDefinition girlPairDef)
    {
    ...
    private static bool CelesteOverride(RelativeId? previousLocId, LocationDefinition locationDef, bool sidesFlipped)
    {
    ...
    private static bool MomoOverride(RelativeId? previousLocId, LocationDefinition locationDef, bool sidesFlipped)
    {
        if (previousLocId == LocationIds.Park && Plugin.ThrewOutGoldfish)
        {
